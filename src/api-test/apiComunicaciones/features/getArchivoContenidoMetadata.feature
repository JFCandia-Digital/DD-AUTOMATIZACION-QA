...
Feature: Pruebas realizadas a la API "GET" - "/comunicaciones/archivo-contenido-y-metadata/:comunicacion_id/:archivo_uuid"
  ...
  Scenario: Obtener contenido del documento principal y anexos usando IDs dinámicos
    ...
    When ejecuto peticiones "GET" a "/comunicaciones/archivo-contenido-y-metadata/:comunicacion_id/:archivo_uuid" para el documento principal y sus anexos
    Then el estado de la respuesta debe ser 200

  Scenario Outline: Validar seguridad del endpoint con distintos tokens
    Given que realizo una petición "GET" a "/comunicaciones/archivo-contenido-y-metadata/86066/3b617cc6-c8c3-4a0e-9c8c-284e7a4ea4f1" con token "<tipo_auth>"
  ...
  Scenario Outline: Validar "GET" - "/comunicaciones/archivo-contenido-y-metadata/:comunicacion_id/:archivo_uuid" campo comunicacionId con distintos tipos de datos
    Given que solicito un token de acceso con el usuario "CLIENT_ID_PDI" y el password "CLIENT_SECRET_PDI"
    Given que realizo una petición "GET" a "/comunicaciones/archivo-contenido-y-metadata/<comunicacion_id>/3b617cc6-c8c3-4a0e-9c8c-284e7a4ea4f1" con token "válido"
    Then el estado de la respuesta debe ser <status>
    And el cuerpo de la respuesta debe tener la estructura de error "<estructura>"
  ...
  Scenario Outline: Validar "GET" - "/comunicaciones/archivo-contenido-y-metadata/:comunicacion_id/:archivo_uuid" campo archivo_uuid con distintos tipos de datos
    Given que solicito un token de acceso con el usuario "CLIENT_ID_PDI" y el password "CLIENT_SECRET_PDI"
    Given que realizo una petición "GET" a "/comunicaciones/archivo-contenido-y-metadata/86066/<archivo_uuid>" con token "válido"
    Then el estado de la respuesta debe ser <status>
    And el cuerpo de la respuesta debe tener la estructura de error "<estructura>"
    And el cuerpo de la respuesta debe tener la propiedad <campo_error> con el valor <mensaje_error_esperado>
